...
function guifi_edit_zone_validate(&$node) {
  ...
  if (($node->maxx != null) && ($node->maxy != null) && ($node->minx != null) && ($node->miny != null)) {
    validate_limits($node->minx, $node->miny,t('Min:'));
    validate_limits($node->maxx, $node->maxy,t('Max:'));
  ...
  // We save upload's file if extension is jpeg.
  if ( $file = file_check_upload('image') ) {
    if ( strcmp( $file->filepath, $node->map ) ) {
      $posExt = strrpos( $file->filename, '.' );
      $ext = strtolower( $posExt ? substr( $file->filename, $posExt + 1 ) : '' );
      if ( !strcmp( $ext, 'jpg') || !strcmp( $ext, 'jpeg') ) {
        // Delete previous file
        if ( file_exists( $node->map) ) {
          file_delete ($node->map);
        }
...
function guifi_zone_print_data($zone) {
  ...
  if ($zone->timestamp_created > 0) 
    $rows[] = array(t('created by'),$name_created->name .'&nbsp;' .t('at') .'&nbsp;' .format_date($zone->timestamp_created)); 
  if ($zone->timestamp_changed > 0) 
    $rows[] = array(t('updated by'),$name_changed->name .'&nbsp;' .t('at') .'&nbsp;' .format_date($zone->timestamp_changed)); 
  $rows[] = array(t('file with the map image'),$zone->image); 
...
function guifi_zone_view(&$node) {
  ...
  switch ($op) {
    ...
    case 'nodes':
      ...
      break;
    case 'map': 
      $output .= guifi_zone_ariadna($node->nid);
