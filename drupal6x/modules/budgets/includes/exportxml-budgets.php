...
echo '<xmlbudgets>';
		...
		if ($qbudgets_cnt > 0) {
		    ...
		    $temporaly_budge_nid=intval($budget['nid']);
				...
				$t = mysql_fetch_array(mysql_query("SELECT sum(quantity * cost) amount FROM budget_items WHERE budget_id = $temporaly_budge_nid",$conn_db));
				$s = mysql_fetch_array(mysql_query("SELECT sum(amount) amount FROM budget_funds WHERE fund_status != 'Declined' AND budget_id = $temporaly_budge_nid",$conn_db));
