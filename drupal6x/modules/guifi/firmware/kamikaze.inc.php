...
function guifi_unsolclic_ospf($dev,$zone) {
  ...
  $ospf_zone = guifi_get_ospf_zone($zone);
    ...
    //  case "100": //* Fonera{2100,2200,+} *//

        _outln_comment();
        _outln_comment(t('Fonera OSPF routing'));
...
function guifi_unsolclic_network_vars($dev,$zone) {
   ...
   if (empty($ntp)) {
    ...
    case "ap": case "AP":
      ...
      _out('	option mode	\'ap\' >> /etc/conf/wireless');
      //_outln_nvram('wl0_mode','ap');
      _out('	option channel	'.$dev->radios[0][channel].' >> /etc/conf/wireless');
      _out('	option ssid	guifi.net-'.guifi_to_7bits($dev->radios[0][ssid]).' >> /etc/conf/wireless');
        ...
        foreach ($interface[ipv4] as $ipv4_id => $ipv4) 
          if (isset($ipv4[links])) foreach ($ipv4[links] as $key => $link) {
            if ($link['link_type'] == 'ap/client') {
              ...
              _out('	option mode	\'sta\'	>> /etc/conf/wireless');
...
function guifi_unsolclic_wds_vars($dev) {
  ...
  _outln_comment();
  _outln_comment('unsolclic version: '.$version.' !! WARNING: Beta version only F o n eras {2100,2200,+}');
//  if ($dev->variable['model_id'] != "100") //* !Fonera{2100,2200,+} *//
//	return;
  ...
  _out('echo "nameserver 10.139.50.1" > /etc/resolv.conf')
