...
function guifi_cnml($cnmlid,$action = 'help') {
  ...
  case 'detail':
   ...
   case 'node':
     ...
     $sql_services = sprintf('SELECT s.*, r.body FROM {guifi_devices} d, {guifi_services} s, {node} n, {node_revisions} r WHERE d.nid=%d AND d.id=s.device_id AND n.nid=s.id AND n.vid=r.vid',$cnmlid);
     break;
   case 'nodecount':
