...
//      '#weight' => $fw2++,   
    ...
    foreach ($radio['interfaces'] as $iid=>$interface)
      $f['interfaces'][$iid] =
        guifi_device_interface_form($interface,array('radios',$key,'interfaces',$iid));
/*    
...
*/
  ...
  // Filter buttons
  $form['dbuttons'] = guifi_device_buttons(true,'guifi_radio_add_link2AP',$form_weight);
...
function guifi_radio_add_wds_confirm_submit(&$form,&$form_state) {
  ...
  $form_state['newinterface'] = $form_state['values']['newInterface']['unfold']=
    true;  
