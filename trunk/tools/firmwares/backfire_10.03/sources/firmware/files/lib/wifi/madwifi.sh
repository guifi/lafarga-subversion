...
scan_atheros() {
	local device="$1"
	local wds
	local adhoc ahdemo sta ap monitor apwds
	...
	for vif in $vifs; do
		...
		case "$mode" in
			...
			;;
                        apwds)
	                        config_set "$vif" apwds 1
	                        config_set "$vif" mode ap
	                        mode="ap"
	                        append $mode "$vif"
                        ;;			
...
config_get apwds "$vif" apwds

case "$apwds" in

	1|on|enable)
		cont=0
		config_get "bssid$cont" "$vif" "bssid$cont"
		eval bssid=\$"bssid$cont"
		while [ $bssid ]
		do
			iwpriv "$ifname" wds_add "$bssid"
