...
***/
function guifi_notification(&$to_mail, $subject, &$message,$verbose = true, $notify = true) {
  ...
  if ($notify) {
    ...
    if ($to_mail != null) {
      drupal_mail(null,
                $to,
                strip_tags('guifi: '.$subject),
                strip_tags(strtoupper($subject)."\n\n".$message),
                'webmestre@guifi.net');
      drupal_set_message(t('A notification has been sent to: %to',array('%to'=>$to)));
    }
  }
...
/***
 * function to UPSERT (SQL insert or update) on
...
/***
 * Delete SQL statements for node, devices, radios, users, services, interfaces, ipv4 and links
***/
function _guifi_db_delete($table,$key,&$to_mail = array(),$depth = 0,$cascade = true) {
  ...
  case 'guifi_devices':
    $item=db_fetch_object(db_query(
      'SELECT d.nick dname, d.contact, d.nid, d.type, d.comment, l.nick nname, l.contact ncontact 
  ...
  case 'guifi_radios':
    $item=db_fetch_object(db_query(
       'SELECT r.protocol, r.ssid sid, r.mode, r.radiodev_counter, d.nick dname, d.contact, d.nid, l.nick nname 
  ...
  $count = db_fetch_array(db_query("SELECT count(*) FROM {".$table."} WHERE ".$where_str));
  if ($count['c'] != 1)
